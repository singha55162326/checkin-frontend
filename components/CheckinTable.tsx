...
const CheckinTable: React.FC<Props> = ({ checkins, onEdit, onDelete }) => {
  ...
  // Format punch_time to show full precision "Nov 4, 2025 • 08:01:00.123456"
  const formatPunchTime = (timestamp: string) => {
    // Handle the PostgreSQL timestamp format: '2025-10-22 17:35:30.694356'
    try {
      ...
      // Format microseconds (ensure 6 digits)
      const formattedMicroseconds = microseconds ? microseconds.padEnd(6, "0").substring(0, 6) : "000000";

      return `${monthName} ${day}, ${year} • ${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}.${formattedMicroseconds}`;
    } catch (error) {
      // Fallback to default date parsing
      const date = new Date(timestamp);
      const day = date.getDate();
            ...
            >
              ...
              <td className="py-4 px-4">
                <div className="flex space-x-2">
                  ...
                  >
                    {deletingId === c.checkin_id ? "Deleting..." : "Delete"}
                  </button>
                </div>
              </td>
            </tr>
